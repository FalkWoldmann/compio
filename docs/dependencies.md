...
rejected, are in [`perf.md`](./perf.md) on the `perf-work`, `alloc-investigation`
and `allocator-eval` branches, and in `queue-design.md` on
`bench-tooling-prototype`.
...
missing dependency: see [`soundness.md`](./soundness.md) on `unsafe-review`.
