...
/// Reads a `T` at `offset`, or fails if the buffer is too short. The field
/// types differ between platforms, so callers let them be inferred.
fn get<T: Pod>(buffer: &[u8], offset: usize) -> Result<T, CodecError> {
    buffer
        .get(offset..)
...
impl AncillaryData for libc::in_pktinfo {
    ...
    fn decode(buffer: &[u8]) -> Result<Self, CodecError> {
        use libc::in_pktinfo as P;
        Ok(libc::in_pktinfo {
            ipi_ifindex: get(buffer, offset_of!(P, ipi_ifindex))?,
            ipi_spec_dst: libc::in_addr {
                s_addr: get(buffer, offset_of!(P, ipi_spec_dst))?,
            },
            ipi_addr: libc::in_addr {
                s_addr: get(buffer, offset_of!(P, ipi_addr))?,
            },
        })
    }
}
...
impl AncillaryData for libc::in6_pktinfo {
    ...
    fn decode(buffer: &[u8]) -> Result<Self, CodecError> {
        ...
        Ok(libc::in6_pktinfo {
            ...
            ipi6_ifindex: get(buffer, offset_of!(P, ipi6_ifindex))?,
        })
    }
}
        ...
        Ok(IN_PKTINFO {
            ipi_addr: WinSock::IN_ADDR {
                S_un: WinSock::IN_ADDR_0 {
                    S_addr: get(buffer, offset_of!(IN_PKTINFO, ipi_addr))?,
                },
            },
            ipi_ifindex: get(buffer, offset_of!(IN_PKTINFO, ipi_ifindex))?,
        })
    }
}
...
impl AncillaryData for IN6_PKTINFO {
    ...
    fn decode(buffer: &[u8]) -> Result<Self, CodecError> {
        Ok(IN6_PKTINFO {
            ...
            ipi6_ifindex: get(buffer, offset_of!(IN6_PKTINFO, ipi6_ifindex))?,
        })
    }
}
