...
impl AsyncRead for RepeatOne {
    async fn read<B: IoBufMut>(&mut self, mut buf: B) -> BufResult<usize, B> {
        // SAFETY: every write below is `MaybeUninit::write` of an initialized
        // byte, so nothing is de-initialized.
...
impl AsyncReadAt for ReadOneAt {
    async fn read_at<T: IoBufMut>(&self, mut buf: T, pos: u64) -> BufResult<usize, T> {
        ...
        if pos > self.0.len() {
            ...
            // SAFETY: every write below is `MaybeUninit::write` of an
            // initialized byte, so nothing is de-initialized.
            let slice = unsafe { buf.as_uninit() };
