...
// A buffer too short for any message rejects the push instead of panicking.
#[test]
fn invalid_buffer_length() {
    let mut buf = AncillaryBuf::<1>::new();
    assert!(matches!(
        buf.builder().push(0, 0, &()),
...
// Empty control data, as `recv_msg` returns for a datagram without any, and an
// unaligned buffer both parse.
#[test]
fn empty_and_unaligned_buffers_parse() {
    assert_eq!(AncillaryIter::new(&[]).count(), 0);
    assert_eq!(AncillaryIter::new(&AncillaryBuf::<64>::new()).count(), 0);

    let mut buf = AncillaryBuf::<{ ancillary_space::<u32>() }>::new();
    buf.builder().push(1, 2, &3u32).unwrap();
    let mut shifted = vec![0u8; buf.len() + 1];
    shifted[1..].copy_from_slice(&buf);
    let msg = AncillaryIter::new(&shifted[1..]).next().unwrap();
    assert_eq!(
        (msg.level(), msg.ty(), msg.data::<u32>().unwrap()),
        (1, 2, 3)
    );
}
